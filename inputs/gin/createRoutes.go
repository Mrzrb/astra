	...
	return func(s *astra.Service) error {
		...
		for _, route := range router.Routes() {
			...
			if file == "<autogenerated>" {
				name := pcFn.Name()
				receiverName, fnName, err := astra.ExtraMethodAndName(name)
				if err != nil {
					continue
				}
				for _, mi := range s.AstMethodInfo {
					if mi.MethodReceiver == receiverName && mi.MethodName == fnName {
						file, _ = filepath.Abs(mi.FileName)

						line = mi.Line
					}
				}
				s.Log.Info().Str("name", name).Msg(name)
			}
